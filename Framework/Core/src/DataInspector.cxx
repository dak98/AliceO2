...
static InputSpec asInputSpec(const OutputSpec& output)
{
...
void addDataInspector(WorkflowSpec& workflow)
{
  ...
  auto inserter = std::back_inserter(dataInspector.inputs);
  for (const DataProcessorSpec& device : workflow) {
    auto& outputs = device.outputs;
    if (isNonInternalDevice(device)) {
      std::transform(outputs.begin(), outputs.end(), inserter, asInputSpec);
    }
  }
